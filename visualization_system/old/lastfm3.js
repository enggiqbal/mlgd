...
import clusterData from './geojson/impred_lastfm2/im_cluster.geojson'
import clusterBoundaryData from './geojson/impred_lastfm2/im_cluster_boundary.geojson'
import edgeyData from './geojson/impred_lastfm2/im_edges.geojson'
import nodeData from './geojson/impred_lastfm2/im_nodes.geojson'
...
if (resolution>2 )
var edgeStyle=new Style({});
  return edgeStyle;
  };

var alledgeSource = new Vector({  url: alledgesData,  format: new GeoJSON() });
var alledgesLayer = new VectorLayer({  source: alledgeSource,  style: alledgeStyleFunction});
...
function getVisible(l,resolution)
{
  ...
  if (l == 2 && resolution< 120) visiable= true;
  if (l == 3 && resolution< 100)  visiable= true;
  if (l == 4 && resolution< 90)   visiable= true;
  if (l == 5 && resolution< 80)  visiable= true;
  if (l == 6 && resolution< 60)  visiable= true;
  if (l == 7 && resolution< 50)  visiable= true;
  if (l == 8 && resolution< 40)  visiable= true;
  return visiable
}
...
var map = new Map({
  ...
  target: 'map',
  view: new View({center:  [12699.907304578861, 10923.33516731054],
      zoom: 10,//12, //17
      maxZoom: 18,
      minZoom: 10  })
});
